...
_TAB_SUMMARIES: dict[str, str] = {
    "general": "日常行为、通知和剪贴板历史。",
    "screenshot": "先设置保存策略，再调整快捷键。",
    "translator": "监听、请求和弹窗都在这里。",
    "proxy": "Git 和终端共用这一组代理设置。",
    "subconverter": "只需配置服务开关和端口。",
    "mihomo": "先选后端，再按需配置 Core 设置和订阅地址。",
}

_TAB_GROUPS: dict[str, list[tuple[str, str, list[str]]]] = {
    "general": [
        ("日常体验", "通知、快捷入口和托盘展示。", [
    ...
    "screenshot": [
        ...
        ("快捷键", "保存后会立即重绑。", [
            ...
            "hotkey_pure_ocr",
            "hotkey_interactive",
            "hotkey_pin_clipboard",
        ]),
    ],
    "translator": [
        ("监听与展示", "哪些输入会触发翻译，以及如何展示结果。", [
            "selection_enabled",
            "clipboard_enabled",
            "popup_enabled",
            "auto_copy",
    ...
    "subconverter": [
        ("本地订阅服务", "为 Clash/Mihomo 提供本地转换服务。", [
            "enable_server",
            "port",
        ]),
    ],
    "mihomo": [
        ("后端选择", "选择 Party 或 Core，以及是否随 DeskVane 自动启动。", [
            "backend",
            "autostart",
        ]),
        ("Core 设置", "仅当后端为 Core 时生效。", [
            "core_binary",
            "core_home_dir",
            "external_controller",
            "secret",
...
# Config group → (display name, icon, dataclass field name on AppConfig)
...
class _SettingsWindow:
    ...
    def _build_sidebar(self) -> None:
        ...
        for idx, (label, icon, _) in enumerate(_TABS):
    ...
    def _render_tab(self, idx: int) -> None:
        ...
        label, icon, attr_name = _TABS[idx]
        sub_config = getattr(self.draft, attr_name)
        ...
        summary = _TAB_SUMMARIES.get(attr_name)
        if summary:
            tk.Label(
                self._body_inner,
                text=summary,
        ...
        for title, desc, field_names in _TAB_GROUPS.get(attr_name, []):
    ...
    def _render_field(
        ...
        if isinstance(value, bool):
            ...
            options = _FIELD_OPTIONS.get(full_key)
            if options:
