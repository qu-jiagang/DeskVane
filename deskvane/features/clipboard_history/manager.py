...
class HistoryOverlay:
    def __init__(self, root: tk.Tk, history: list[str], on_select: callable):
        ...
        x = max(0, min(px - (w // 2), sw - w))
        y = max(0, min(py - (h // 2), sh - h))
        self.top.geometry(f"{w}x{h}+{x}+{y}")

        lbl = tk.Label(self.top, text="📋 剪贴板历史记录", bg="#2d2d2d", fg="#6366f1", font=("sans-serif", 10, "bold"), anchor="w")
        lbl.pack(fill=tk.X, padx=10, pady=(10, 5))
        instr = tk.Label(self.top, text="使用 ↑ ↓ 浏览，回车选择 / 快捷键 1-9 秒选 / Esc 退出", bg="#2d2d2d", fg="#888888", font=("sans-serif", 8), anchor="w")
...
class ClipboardHistoryManager:
    ...
    def _flush_to_disk(self) -> None:
        ...
        try:
            ...
            self._HISTORY_PATH.write_text(json.dumps(self.history[:self._MAX_ENTRIES], ensure_ascii=False), encoding="utf-8")
        except Exception:
            pass
    ...
    def _on_select(self, index: int):
        if 0 <= index < len(self.history):
            ...
            if not self._write_clipboard_text(content):
    ...
    def _read_clipboard_text(self) -> str | None:
        try:
            content = self.app.platform_services.clipboard.read_text("clipboard")
            if content is not None:
                return content
        except Exception:
            pass
        try:
            return self.app.root.clipboard_get()
        except tk.TclError:
            return None

    def _write_clipboard_text(self, content: str) -> bool:
        try:
            return bool(self.app.platform_services.clipboard.write_text(content))
        except Exception:
            return False
