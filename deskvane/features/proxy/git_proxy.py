...
class GitProxyManager:
    ...
    def enable(address: str) -> None:
        subprocess.run(["git", "config", "--global", "http.proxy", address], check=True, capture_output=True)
        subprocess.run(["git", "config", "--global", "https.proxy", address], check=True, capture_output=True)

    @staticmethod
    def disable() -> None:
        for key in ("http.proxy", "https.proxy"):
            subprocess.run(["git", "config", "--global", "--unset", key], capture_output=True)
